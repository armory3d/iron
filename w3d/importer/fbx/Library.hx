...
using wings.w3d.importer.fbx.Data;
...
class Library {
	...
	**/
	...
	public function makeObject( ?textureLoader:Void->/*String -> FbxNode ->*/ wings.w3d.material.Material) : wings.w3d.Object {
		...
		if( textureLoader == null ) {
			...
			//	return new h3d.mat.MeshMaterial(tmpTex);
				return new wings.w3d.material.TextureMaterial(com.luboslenco.test.R.shader, com.luboslenco.test.R.box);
			}
		}
		// create all models
		for( model in root.getAll("Objects.Model") ) {
			...
			case "Mesh":
				...
				if( prim == null ) {
					prim = new wings.w3d.mesh.FbxGeometry("", new Geometry(this, g));
		...
		for( o in objects ) {
			...
			if( rootJoints.length != 0 ) {
				...
				if( skinData.boundJoints.length > maxBonesPerSkin )
					skinData.split(maxBonesPerSkin, Std.instance(skinData.primitive, wings.w3d.mesh.FbxGeometry).geom.getIndexes().vidx);
	...
	function createSkin( hskins : Map<Int, wings.w3d.anim.Skin>, hgeom : Map<Int, wings.w3d.mesh.FbxGeometry>, rootJoints : Array<wings.w3d.anim.Skin.Joint>, bonesPerVertex ) {
		var allJoints = [];
