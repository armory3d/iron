...
class Vec4 {
	...
	inline public function inverse():Vec4 {
		x = -x;
		y = -y;
		z = -z;
		return this;
	...
	inline public function slerp(from:Vec4, to:Vec4, s:FastFloat):Vec4 {
	...
	inline public function project(v:Vec4) {
		var dot = this.dot(v);
	...
	inline public function projectOnPlane(n:Vec4) {
		var dot = this.dot(n);
	...
	inline public static function fastOrthoNormalize(normal:Vec4, tangent:Vec4) {
		normal.normalize();
		tangent.sub(helpVec0.setFrom(tangent).project(normal));
		tangent.normalize();
	}

	inline public static function orthoNormalize(normal:Vec4, tangent:Vec4, binormal:Vec4) {
		normal.normalize();
		tangent.sub(helpVec0.setFrom(tangent).project(normal));
