...
class DefaultMatrixes {
	...
	public static inline function rightHandToLeft( m : wings.math.Mat4 ) {
...
class Library {
	...
	**/
	...
	public function loadAnimation( mode : AnimationMode, ?animName : String, ?root : FbxNode, ?lib : Library ) : wings.w3d.anim.Animation {
		if( lib != null ) {
		...
		switch( mode ) {
		case FrameAnim:
			var anim = new wings.w3d.anim.FrameAnimation(animName, numFrames, sampling);
		...
		case LinearAnim:
			...
			var anim = new wings.w3d.anim.LinearAnimation(animName, numFrames, sampling);
			var q = new Quat(), q2 = new Quat();
			...
			for( c in curves ) {
				...
				for( f in 0...numFrames ) {
					...
					if( changed ) {
						var f = new wings.w3d.anim.LinearAnimation.LinearFrame();
						if( c.s == null || sp == 0 ) {
							if( def.scale != null ) {
								f.sx = def.scale.x;
								f.sy = def.scale.y;
	...
	public function makeObject( ?textureLoader:Void->/*String -> FbxNode ->*/ wings.w3d.materials.Material) : wings.w3d.Object {
		var scene = new wings.w3d.Object();
		...
		if( textureLoader == null ) {
			...
			//	return new h3d.mat.MeshMaterial(tmpTex);
				return new wings.w3d.materials.TextureMaterial(com.luboslenco.test.R.shader, com.luboslenco.test.R.box);
			}
		}
		// create all models
		for( model in root.getAll("Objects.Model") ) {
			var o : wings.w3d.Object;
			var name = model.getName();
			if( skipObjects.get(name) )
				continue;
			...
			case "Null", "Root", "Camera":
				...
				if( hasJoint )
					o = new wings.w3d.scene.Skin(null, null, scene);
				else
					o = new wings.w3d.Object(scene);
			case "LimbNode":
				var j = new wings.w3d.anim.Skin.Joint();
			...
			case "Mesh":
				...
				if( prim == null ) {
					prim = new wings.w3d.meshes.FbxGeometry("", new Geometry(this, g));
				...
				while( tmats.length > lastAdded )
					tmats.pop();
				if( tmats.length == 0 )
					//tmats.push(new wings.w3d.materials.Material(h2d.Tile.fromColor(0xFFFF00FF).getTexture()));
					tmats.push(textureLoader());
				// create object
				if( tmats.length == 1 )
					o = new wings.w3d.scene.Model(new wings.w3d.meshes.Mesh(prim, tmats[0]), scene);
				else {
		...
		for( o in objects ) {
			...
			if( rootJoints.length != 0 ) {
				if( !Std.is(o.obj, wings.w3d.scene.Skin) )
					throw o.obj.name + ":" + o.model.getType() + " should be a skin";
				var skin : wings.w3d.scene.Skin = cast o.obj;
				...
				for( osub in objects ) {
					if( !Std.is(osub.obj, wings.w3d.scene.Model)) continue;
				...
				if( skinData.boundJoints.length > maxBonesPerSkin )
					skinData.split(maxBonesPerSkin, Std.instance(skinData.primitive, wings.w3d.meshes.FbxGeometry).geom.getIndexes().vidx);
	...
	function keepJoint( j : wings.w3d.anim.Skin.Joint ) {
		return keepJoints.get(j.name);
	}
	
	function createSkin( hskins : Map<Int, wings.w3d.anim.Skin>, hgeom : Map<Int, wings.w3d.meshes.FbxGeometry>, rootJoints : Array<wings.w3d.anim.Skin.Joint>, bonesPerVertex ) {
		var allJoints = [];
		function collectJoints(j:wings.w3d.anim.Skin.Joint) {
		...
		for( j in iterJoints ) {
			...
			if( skin == null ) {
				...
				skin = new wings.w3d.anim.Skin(geom.getVerticesCount(), bonesPerVertex);
				geom.skin = skin;
				skin.primitive = geom;
				hskins.set(def.getId(), skin);
			}
			j.transPos = wings.math.Mat4.L(subDef.get("Transform").getFloats());
