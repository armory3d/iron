...
class Emitter extends Trait implements IUpdateable implements IRenderable2D {
        ...
        while (ii < numParticles) {
            ...
            if (particle.life > dt) {
                if (gravityType) {
                    ...
                    particle.x = transform.absx /*emitX*/ - Math.cos(particle.radialRotation) * radius;
                    particle.y = transform.absy /*emitY*/ - Math.sin(particle.radialRotation) * radius;
	...
	function initParticle(particle:Particle):Bool {
        ...
        if (type == Gravity) {
            ...
            particle.y = random(transform.absy /*emitY*/, emitYVariance);
